...
Thm.~\ref{thm:S} & Kneser's theorem + Lem.~\ref{lem:mono}; (iii) upgrade at $R \le 107$: enumeration & \S\ref{sec:sieve} & Lean, part (i); (iii) clause: Python \\ \addlinespace
...
Python layer and the hand proofs alone. Theorem~\ref{thm:R15}'s
finite check and Theorem~\ref{thm:S}(i) \emph{are} formalized: the
former by a product-index mask bridge over the non-cyclic
$(\mathbb{Z}/15)^* \cong C_2 \times C_4$, the latter fully
symbolically --- Kneser's addition theorem vendored from its
existing sorry-free formalization, an iterated many-summand form
proved on top, and the support bound derived on standard axioms
only, no evaluator appeal.) For these
