...
Lem.~\ref{lem:S} & Thm.~\ref{thm:S}; independently, Lem.~\ref{lem:mono} + enumerations / DP & \S\ref{sec:sieve} & Lean: as a corollary of Thm.~\ref{thm:S} at every prime $R$; directly at $R = 19, 23, 31$ \\ \addlinespace
...
\begin{remark}[data revision in progress]\label{rem:erratum}
A defect was found in the factorization routine used for the
$10^{12}$ run \emph{after} the results below were first recorded, and
this section is being revised against a corrected recomputation. The
routine factors $a = (p+R)/4$ by trial division against a fixed table
of primes below $180{,}000$; that certifies the surviving cofactor
prime only while $a < 180{,}000^2$, i.e.\ while $p < 1.296 \times
10^{11}$. Above that cliff a cofactor which is a product of two primes
both beyond the table was recorded as prime, so the divisor set of
$m^2$ came out incomplete.

The error is \emph{one-directional}, and that bounds the damage
exactly. Every divisor the routine does find is genuine and every
certificate is exact-checked, so a residual it reports as working
really does work; what it can do is fail to notice that a
\emph{smaller} residual also works. Hence the recorded $R_{\min}$ is
$\ge$ the true $R_{\min}$ at every prime, with equality guaranteed
below the cliff. Three consequences.
\textup{(i)}~Existence is untouched: every hard prime below $10^{12}$
has an exhibited, exactly verified certificate, and
$\mathrm{num\_unsolved} = 0$ stands.
\textup{(ii)}~The record stands, and in fact is now
\emph{confirmed}: the maximum of the true $R_{\min}$ is at most the
maximum of the recorded values, which is $111$; and it is attained,
because the prime realizing $111$ lies at
$1.199 \times 10^{11}$, below the cliff, where the factorization is
complete --- so $\max R_{\min} = 111$ exactly over hard primes below
$10^{12}$. What is not yet confirmed is the \emph{number} of primes
attaining it; the two others recorded at $111$ lie above the cliff.
\textup{(iii)}~The distribution of Table~\ref{tab:dist} and the
deep-tail counts are affected above the cliff, which is $86\%$ of the
range: a window census at $10^{12}$ finds $31$ of $2{,}282$ hard
primes ($1.4\%$) with an overstated residual, so of order $10^7$
entries are expected to move --- always downward, from the tail toward
the head. Everything at or below $10^{11}$ is unaffected, the cliff
lying above it; that includes the $10^9$ solvability masks and hence
the calibration of Section~\ref{sec:calib}, the failure taxonomy, and
the band-filling test of the model, whose primes all lie in
$(1.3 \times 10^{10}, 10^{11})$.
The defect is fixed in the accompanying repository
(\texttt{bulk\_generate.factorize} now completes the cofactor by
Miller--Rabin and Pollard-rho, with a regression test on the primes
that exposed it), and the $10^{12}$ census is being regenerated.
Figures marked \emph{provisional} below are the ones awaiting that
recomputation.
\end{remark}
...
with full minimality --- but see Remark~\ref{rem:erratum}: above
$1.296 \times 10^{11}$ the minimality re-check shared the defective
factorization and so is not independent of it; it remains a valid
check of the certificate itself. Included are the three entries at
...
\emph{Provisional above $p = 1.296 \times 10^{11}$}: see
Remark~\ref{rem:erratum}. Counts there are upper bounds for the tail
and lower bounds for the head; the totals and the maximum are
unaffected.}
\label{tab:dist}
\end{table}
...
\item \textbf{The maximum moves at $10^{12}$.} Throughout
  ...
  $R_{\max} = 111$ --- confirmed exactly, by
  Remark~\ref{rem:erratum}(ii) --- recorded as attained by three
  primes (a count still to be confirmed), the least being
  \[
...
\item \textbf{A prediction, and what it was worth.} The \emph{calibrated
  ...
  there, at $R = 111$, at a prime below the cliff of
  Remark~\ref{rem:erratum} and hence correctly computed. The deep
  tail is recorded as holding $73$ primes with $R_{\min} \ge 87$
  against $19$ a decade earlier, distributed $40, 5, 12, 4, 5, 4, 3$
  over $87, \dots, 111$ --- \emph{provisional}: $22$ of the $73$ lie
  below the cliff and are confirmed, the rest await recomputation and
  can only move downward. A model fitted at
...
\subsection{Calibration of the base case of Theorem~\ref{thm:FG}}\label{sec:calib}
...
injectivity, and an explicit $2^{c\sqrt d}$ lower bound.

The transfer of Theorem~\ref{thm:S}(ii) out of the additive model is
formalized as well, which makes Lemma~\ref{lem:S} a machine-checked
\emph{corollary} of Theorem~\ref{thm:S} rather than a separately
verified statement. The chain is: membership in the budget-2 reach
yields an explicit exponent decomposition with all exponents
$\le 2$; a set of units of $\mathbb{Z}/R$ avoiding the identity
carries at most one involution, since $x^2 = 1$ has only the roots
$\pm 1$ in a field, which collapses the general bound
$\max(\lfloor (g+t-2)/2\rfloor, g/2-1)$ to $g/2 - 1$ at
$G = (\mathbb{Z}/R)^*$ exactly as a single $2$-torsion class does in
the cyclic case; hence at least $(R-1)/2$ factor classes other than
the identity reach every unit class as a bounded-exponent product,
which is realized by an honest integer divisor of $a^2$ (a product of
bounded prime powers, dividing $a^2$ by pairwise coprimality); and
composing with the divisor bridge gives, for \emph{every} prime
residual $R < p$ with $4a = p+R$, explicit positive $k \mid (pa)^2$,
$b$, $c$ satisfying $4abc = p(bc+ac+ab)$. The $R = 19$ composed
corollary is the same conclusion at one residual, inheriting the
\texttt{native\_decide} axiom of the enumeration behind it; the
general statement is symbolic throughout --- no enumeration, no
evaluator, no restriction on $R$. (Still not formalized: the identity
$t = 2^{\omega(R)}-1$ for the involution count at \emph{composite}
$R$, so the general bound is quoted there with that $t$ supplied by
hand; the \emph{iterated} kernel reduction; and the analytic
arguments of Sections~\ref{sec:sieve} and~\ref{sec:ladder}, which
rest on the hand proofs alone.) For these
