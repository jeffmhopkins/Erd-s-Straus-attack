...
residual $R$ is a computable function of the factor classes of $(p+R)/4$
...
verified certificates for all $128{,}671{,}219$ hard primes below
$10^{11}$; the maximal minimal residual is $107$, unchanged from
$5 \times 10^7$, and the calibrated model correctly predicted the
population of the last empty band of minimal residuals between $10^{10}$
and $10^{11}$.
\end{abstract}
...
\cite{Swett} and to $10^{17}$ by Salez \cite{Salez}; the present paper
is logically independent of those bounds. The structure of
...
quantitative structural reduction. For calibration, Vaughan's classical
...
($\equiv 169 \bmod 840$) and the smallest admissible residual $R = 3$.
...
single global statement about $p$ — the structural explanation of joint
failure across residuals:
...
The sieve consequences need one input per residual: a ``sieve-grade
handle'' making failure at $R$ a positive-density multiplicative
restriction. For $R = 3, 7, 11$ the handles are the exact criteria; for
larger $R$ it is the following machine-verified lemma
...
\emph{Organization.} Proofs are arranged by logical dependency, which
matches the statement order above: Section~\ref{sec:obstruction}
develops the obstruction framework (Propositions~\ref{prop:char},
\ref{prop:succ} and the proof of Theorem~\ref{thm:J});
...
Theorem~\ref{thm:FG}, Lemma~\ref{lem:S},
Proposition~\ref{prop:agg}, and Theorem~\ref{thm:D}.
...
single global statement about $p$, and is the structural explanation of
...
$\smash{\legendre{p}{q}} = +1$ for every unforced odd prime $q$
dividing any of the shifted values --- \emph{one condition per distinct
prime, shared across every residual it divides}. This is the
correlation mechanism behind the deep tail: smooth shifted values have
fewer distinct prime factors, hence fewer independent sign conditions.
For the record prime $8{,}803{,}369$, the $27$ shifted values contain
only $43$ distinct odd primes (typical: $45$--$52$) and $81\%$ of them
satisfy $\legendre{p}{q} = +1$ (typical: $44$--$60\%$) --- a
$\approx 4\sigma$ fluctuation in provably the right statistic.
Verified: $135{,}588$ instances of Theorem~\ref{thm:J} and $61{,}438$
forced-prime instances in the $10^9$ data, zero violations of either.
\end{corollary}
...
$k = a p^2$ divides $m^2 = p^2 a^2$ and $k \equiv -m$ iff
$a p^2 \equiv -p a$, i.e.\ $p \equiv -1 \pmod R$; $k = a^2 p$ divides
$m^2$ and $a^2 p \equiv -p a$ iff $a \equiv -1$, i.e.\
...
$10^9$, and none of the four critical primes.
\end{proof}
...
\item $R_{\max} = 107$, attained by the single prime $8{,}803{,}369 <
  10^7$, \emph{unchanged from $5 \times 10^7$ to $10^{11}$}.
\item \textbf{A confirmed prediction.} Below $10^{10}$ no prime had
  ...
  with probabilities $0.53, 0.11, 0.22, 0.03, 0.05, 0.02$. Between
...
\item \textbf{The constant $5.17$.} The joint $\{3,7\}$-exceptional
  ...
  --- open, and a natural test problem for such methods.
\item \textbf{Structural reformulations and complexity.} Theorem~\ref{thm:J}
  ...
  $\approx 2\times$ per successive prime residual ($271$ at $R = 19$ to
...
\item \textbf{The record and the covering hypothesis.}
  ...
  incremental item: the residual formulation is \emph{complete} (any
  solution with denominators $a \le b \le c$ has $p/4 < a \le 3p/4$,
  so $R = 4a - p \in (0, 2p]$ with $R \equiv 3 \pmod 4$ automatic),
  hence $R_{\min}(p) \le 2p$ is \emph{equivalent} to the conjecture at
  $p$. An unconditional bound $R_{\min}(p) \le f(p)$ for any $f$
  whatsoever is the full problem.
\end{enumerate}
