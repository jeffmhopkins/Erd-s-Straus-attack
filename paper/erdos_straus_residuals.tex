...
\item A machine-checked (Lean~4/mathlib) verification of the elementary
  ...
  (Lemma~\ref{lem:mono}), the full $1{,}536$-configuration verification
  of Theorem~\ref{thm:Ap}, and the support-bound check of
  Lemma~\ref{lem:S} at $R = 19$ --- in the accompanying repository.
\end{itemize}
...
configuration counts and entry points. Two of the enumerations are
additionally formalized in Lean~4: the $R = 7$ verification of
Theorem~\ref{thm:Ap} (re-stated multiplicatively over
$(\mathbb{Z}/7)^\times$) and the $R = 19$ case of Lemma~\ref{lem:S}
(in the discrete-log coordinates of its proof) are discharged by
Lean's compiled evaluator, with the monotonicity reductions
(Lemma~\ref{lem:mono}) proved as theorems (Appendix~\ref{app:verif}).
\end{remark}
...
multiplicative model itself) and the $R = 19$ case of
Lemma~\ref{lem:S} ($437{,}580$ checks, stated in the discrete-log
coordinates in which the proof and \texttt{verify\_support\_bound}
work) are discharged by Lean's compiled evaluator
