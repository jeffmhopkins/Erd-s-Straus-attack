...
density bounds on the exceptional set are due to Webb~\cite{Webb}, and
Vaughan's classical
...
configuration counts and entry points. The enumerations are
additionally formalized in Lean~4, with the reductions they rely on
proved as theorems rather than trusted; Appendix~\ref{app:verif}
gives the inventory and the exact trust base.
\end{remark}
...
(its late population is analyzed below); the record $R = 107$ is
...
\item $R_{\max} = 107$, attained by the single prime $8{,}803{,}369 <
  ...
  --- a $\approx 4\sigma$ fluctuation in the statistic that
...
$R = 3$ and $R = 7$, multiplied by $\log x$, in five logarithmically
spaced bins of the $10^9$ data. Constancy of the product is the decay
...
repository (\texttt{lean/}) in Lean~4 with its mathematical library
mathlib. For readers unfamiliar with proof assistants: Lean is a
...
\item \emph{Two capstones}: a composed corollary (if $(p+19)/4$ has
  ...
  support, and the evaluator's only task is the final check that
  every state realized by $\ge 15$ classes hits all $30$ targets
  ($3{,}001$ states in the run).
\end{itemize}
Every main theorem is audited with \texttt{\#print axioms}, and
helper lemmas are covered transitively through them. For these
...
\texttt{\ \ forced\_logs=[\_dlog\_table(11)[3]])} from
