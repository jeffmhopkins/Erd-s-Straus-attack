...
Thm.~\ref{thm:Mcount} & explicit construction (complete partitions) & \S\ref{sec:sieve} & Lean, the completeness lemma \\ \addlinespace
...
only, no evaluator appeal. So too the branch-classification layer:
...
container size bound $\le d/2 - 1$ derived from
Theorem~\ref{thm:S}(i) and its odd-modulus variant downstairs;
Proposition~\ref{prop:betavac} in full; and the completeness lemma
behind Theorem~\ref{thm:Mcount} (budget-$2$ subset sums of a
complete partition are exactly $[0, 2N]$) together with the
singleton-miss mechanism it yields. What is left unformalized there
is the \emph{iterated} reduction --- the strong induction composing
kernel steps --- and the $2^{c\sqrt d}$ counting.) For these
