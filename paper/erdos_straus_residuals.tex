...
$p \le x$ satisfy $R_{\min}(p) \le \varepsilon \log\log x$.
...
\item A constructive sieve chain built on these criteria
  ...
  $R_{\min}(p) \le \varepsilon\log\log x$, with the three walls that
  make $\exp(-c(\log\log x)^2)$ the ceiling of the method identified
...
\begin{remark}[the ceiling, and what lies beyond]\label{rem:ceiling}
Theorem~\ref{thm:U} sits at the exact boundary of this method
family: the branch factor $e^{\Theta(B^2)}$ against the sieve gain
$e^{-BL/10}$ forces $B \ll L$, and independently the class modulus
$\widetilde M = e^{O(B)}$ and the Mertens-in-progressions inputs
leave their valid ranges at $B \gg L$ --- three walls detonating at
the same scale, so $\exp(-cL^2)$ is the honest unconditional
frontier here even under substantial improvements to the branch
count. Vaughan's bound $x\exp(-c(\log x)^{2/3})$~\cite{Vaughan}
remains far stronger as a pure density statement; what
Theorem~\ref{thm:U} adds is structural: an explicit certificate at a
residual of size $O(\log\log x)$ for almost every hard prime, a
statement mean-value methods do not address. Passing the
$\exp(-cL^2)$ ceiling toward Vaughan and beyond is exactly what the
measured hypotheses of the ladder program
(Section~\ref{sec:ladder}) would buy: the calibrations
$J = (\log x)^{\theta}$ of Theorem~\ref{thm:L1} are out of reach of
every unconditional variant considered here.
\end{remark}
...
\item \textbf{Beyond fixed lists.} The support bound is proved
  ...
  (Theorem~\ref{thm:S}), and Theorem~\ref{thm:U} shows the two
  walls of list growth --- per-branch forbidden classes depending on
  $p \bmod R$, and the growing branch count --- are affordable up to
  lists of length $\varepsilon\log\log x$, but no further
  (Remark~\ref{rem:ceiling}); the fixed-list format itself cannot
  reach emptiness (Remark~\ref{rem:emptiness}). Passing the ceiling
  by structure rather than hypothesis would need a poly($R$)
  \emph{branch classification}: every failing configuration
  contained in polynomially many kernel or coset-progression
  branches --- a Kneser/Freiman-type structure problem for
  bounded-exponent product sets, whose $\omega(R) \le 2$ extremal
  cases are exactly Theorem~\ref{thm:S}(i)/(iii) and the
  \texttt{kneser\_support\_general} data. The proof of Theorem~\ref{thm:S} shows the extremal
