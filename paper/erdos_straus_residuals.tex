...
the bounds on $R_{\min}$, the exact local criteria that produce
them, and the certificates they exhibit have no counterpart in the
mean-value literature. Vaughan's theorem is the benchmark against
...
\item The identification of Type-I failure families as
...
comparison does not arise: no bound on $R_{\min}$ was previously
available by any method.
\end{remark}
...
The key to counting true failures is that the Type-I failure
