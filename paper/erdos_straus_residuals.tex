...
\begin{remark}[a corrected recomputation]\label{rem:erratum}
The figures below come from a second, independent computation of the
census. The first used a factorization routine that trial-divides
$a = (p+R)/4$ against a fixed table of primes below $180{,}000$; that
certifies the surviving cofactor prime only while $a < 180{,}000^2$,
i.e.\ while $p < 1.296 \times 10^{11}$. Above that cliff a cofactor
which is a product of two primes both beyond the table was recorded as
prime, so the divisor set of $m^2$ came out incomplete.

The resulting error was \emph{one-directional}, which is what made the
damage assessable before the recomputation existed. Every divisor the
routine finds is genuine and every certificate was exact-checked, so a
residual reported as working really does work; what the routine could
do is fail to notice that a \emph{smaller} residual also works. Hence
the first run's $R_{\min}$ was $\ge$ the true value at every prime,
with equality guaranteed below the cliff. Three consequences, all
since confirmed by the recomputation.
\textup{(i)}~Existence was never in question: every hard prime below
$10^{12}$ had an exhibited, exactly verified certificate, and
$\mathrm{num\_unsolved} = 0$ in both runs.
\textup{(ii)}~The record was already determined. The maximum of the
true $R_{\min}$ is at most the maximum of the recorded values, which
was $111$; and it is attained, because the prime realizing $111$ lies
at $1.199 \times 10^{11}$, below the cliff, where the factorization is
complete. So $\max R_{\min} = 111$ followed from the defective run
alone, and the recomputation reproduces it at the same prime.
\textup{(iii)}~The distribution moved, in the predicted direction and
by the predicted mechanism: $5{,}748{,}517$ primes --- $0.49\%$ of the
census --- had an overstated residual, every one of them corrected
downward, with the count at $R = 3$ rising by exactly the total lost
by all larger residuals, and the cumulative counts
$\#\{R_{\min} \le t\}$ increasing at every threshold $t$, as
one-directionality requires. The deep tail proved almost entirely
real: of the $73$ primes recorded with $R_{\min} \ge 87$, $72$
survive; the single casualty is $p = 565{,}158{,}121{,}441$, recorded
at $R = 107$ and in fact at $R = 7$. The three primes attaining $111$
are unchanged.

Both runs are archived. The defect is fixed in the accompanying
repository --- \texttt{factorize} now completes the cofactor by
Miller--Rabin and Pollard-rho, so its correctness no longer depends on
the range --- with a regression test on the primes that exposed it.
The recomputation was performed by an independent implementation (a
windowed divide-as-you-sieve factorization, complete by construction);
the two agree byte-for-byte below the cliff, and the corrections above
it are the difference.
\end{remark}
...
checked; \emph{all} $114{,}930$ tail entries ($R \ge 43$) checked
with full minimality, including the three attaining the record
$R = 111$; zero unsolved; both arrays pinned by sha256; independently
recomputed (Remark~\ref{rem:erratum}), regenerated by
\texttt{scripts/run\_1e12.sh} \\
\bottomrule
\end{tabular}
...
$R \ge 51$, $20{,}151$ and $114{,}930$ with $R \ge 43$) re-checked
with full
...
3 & 663{,}393{,}669 & 43 & 42{,}461 & 83 & 76 \\
7 & 284{,}317{,}352 & 47 & 48{,}679 & 87 & 40 \\
11 & 164{,}898{,}255 & 51 & 8{,}571 & 91 & 5 \\
15 & 32{,}783{,}167 & 55 & 7{,}208 & 95 & 12 \\
19 & 18{,}905{,}009 & 59 & 4{,}705 & 99 & 4 \\
23 & 7{,}807{,}383 & 63 & 1{,}398 & 103 & 5 \\
27 & 1{,}351{,}293 & 67 & 476 & 107 & 3 \\
31 & 1{,}284{,}828 & 71 & 938 & 111 & 3 \\
35 & 203{,}486 & 75 & 143 &  &  \\
39 & 156{,}024 & 79 & 203 &  &  \\
\bottomrule
\end{tabular}
...
These are the corrected counts of Remark~\ref{rem:erratum}.}
\label{tab:dist}
\end{table}
...
\item \textbf{A prediction, and what it was worth.} The \emph{calibrated
  ...
  there, at $R = 111$. The deep tail now holds $72$ primes with
...
wall-clock & $10^{12}$ generation: $1.5$\,h for the corrected run
(native windowed factorization); the first, defective run took
$10.5$\,h on $32$ cores (AMD Ryzen AI Max+ 395); $10^{11}$:
...
samples plus all $114{,}930$ tail entries at $10^{12}$
