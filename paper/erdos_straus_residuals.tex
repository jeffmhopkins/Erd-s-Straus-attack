...
conjecture far beyond any range considered here (to $10^{14}$ by Swett, to
$10^{17}$ by Salez \cite{Salez}, and further in recent work%
\footnote{Verification to $10^{18}$ is reported in recent literature; the
present paper is logically independent of that bound. % TODO: cite the
% 2025 verification and the 2026 residual-certificate notes when finalizing.
}); the structure of the solution count was analyzed by Elsholtz and Tao
\cite{ElsholtzTao}. Verification, however, exhibits no mechanism; the aim
here is a quantitative structural reduction.
...
Each computer-assisted component below is a deterministic, exhaustive
enumeration of a finite space whose sufficiency is established in the
text; all arithmetic is exact integer arithmetic, each run completes in
seconds on commodity hardware, and the entry points named are in the
module \texttt{erdos\_straus.theory} of the accompanying repository.
...
\texttt{finite\_criterion\_dp} \\ \hline
Lem.~\ref{lem:S} ($R=19$) &
...
% TODO before submission: add the 2025 verification-to-10^18 reference and
% the 2026 residual-certificate notes; confirm the Swett 10^14 citation.
