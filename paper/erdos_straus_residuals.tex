...
(iii)~an unconditional sieve bound: the number of hard primes $p \le x$
solved by neither $R=3$ nor $R=7$ is $O\!\left(x/(\log x)^2\right)$, and
with $R = 11$ included, $O\!\left(x/(\log x)^{5/2}\right)$; (iv)~a general
...
With Theorem~\ref{thm:App} the count of hard primes with
$R_{\min}(p) > 11$ is $O\!\left(x/(\log x)^{5/2}\right)$. In particular
these exceptional sets have relative density
$O((\log x)^{-1})$, resp.\ $O((\log x)^{-3/2})$, among hard primes.
\end{theorem}
...
For the second assertion, Theorem~\ref{thm:App} adds the form
$(p+11)/4 = n + 2$ --- \emph{three consecutive integers} --- with the
case-(a) condition ``no prime factor among the five non-residue classes
mod $11$'' (dimension $\tfrac12$); the case-(b) configurations demand all
factors of $n+2$ within $4$ of the $10$ unit classes, a condition of
strictly higher dimension ($\tfrac{6}{10}$), hence negligible. Total
dimension $\tfrac12 + \tfrac12 + \tfrac12 + 1 = \tfrac52$.
\end{proof}
...
\item \textbf{More exact criteria.} The meta-theorem makes every fixed
  $R$ decidable; $R = 15$ (first composite, coupled conditions mod $3$
  and $5$) and $R = 19, 23$ are natural next cases, each raising the
  proven exponent in the chain of Theorem~\ref{thm:E}.
