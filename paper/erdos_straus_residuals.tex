...
certificates and exact local criteria. Computationally we exhibit
...
\item A machine-checked (Lean~4/mathlib) verification of the elementary
  ...
  by certified dynamic programming), the proved
  discrete-log bridges carrying each enumeration into the
  multiplicative model, and the reduction of Theorem~\ref{thm:meta}
  itself (reachability in the model is equivalent to the existence of
  a divisor certificate \eqref{eq:cert}, and produces one explicitly)
...
Lean's compiled evaluator, with the monotonicity reductions
(Lemma~\ref{lem:mono}) \emph{and} the discrete-log coordinate
translations proved as theorems, so that all four enumerations hold
as machine-checked statements directly in the multiplicative model
...
Theorem~\ref{thm:J}, whose prerequisites live there.
...
$\smash{\legendre{2}{R}} = +1$): \emph{the small primes forced into
the shifted values by the Mordell structure never break a Type-I
failure} --- a conceptual explanation of why the hard classes are
exactly the square classes. (ii) $p$ fails Type-I at every prime
...
\emph{shifted values} $(p+R)/4$, $R \le B$ --- \emph{one condition per
distinct prime, shared across every residual it divides}. Failures at
distinct residuals therefore correlate through shared prime factors:
shifted values with few distinct prime factors leave few independent
sign conditions. Section~\ref{sec:comp} quantifies this for the record
prime. Both parts are verified at scale ($135{,}588$ instances of
Theorem~\ref{thm:J}, $61{,}438$ forced-prime instances, zero violations
of either; Appendix~\ref{app:verif}).
\end{corollary}
...
$4^6$ multisets before the structural constraints of
Section~\ref{sec:exact} and $1{,}536$ consistent configurations after
them; at $R = 11$ it compresses to $25$ equivalence states. The
...
\section{Computations to $10^{11}$}\label{sec:comp}
...
sieve exponent of Theorem~\ref{thm:FG}'s base case is exactly right, and $5.17$
...
\item \textbf{The constant $5.17$.} The joint $\{3,7\}$-exceptional
...
support-bound check ($R = 107$: $331$\,s); the entry points
...
--- the enumeration code itself, roughly sixty lines per check. Three
...
and we would welcome one. Fourth, part of the enumeration layer is
formalized in Lean~4/mathlib in the accompanying repository
(\texttt{lean/}): the reachability model underlying
Theorem~\ref{thm:meta} is defined over $(\mathbb{Z}/R)^\times$
directly, both halves of Lemma~\ref{lem:mono} are proved as theorems
(\texttt{reach\_mono}, \texttt{reach\_sublist}) with no computational
content to trust, and the full enumerations behind
Theorem~\ref{thm:Ap} ($1{,}536$ configurations), Theorem~\ref{thm:App}
($497{,}664$ capped configurations, including the exact two-case
failure classification), and the $R = 19, 23$ cases of
Lemma~\ref{lem:S} ($437{,}580$ resp.\ $7{,}759{,}752$ checks) are
discharged by Lean's compiled evaluator (\texttt{native\_decide},
which trusts the Lean compiler in addition to the kernel). The three
larger enumerations are \emph{evaluated} in the discrete-log
coordinates in which the proofs and \texttt{verify\_support\_bound}
work, but the coordinates add nothing to the trusted base: the
translation is itself a proved theorem (the mask semantics ---
rotation is multiplication by $g^s$, the mask fold is the
divisor-class reachability set), and the formal development derives
each statement back in the multiplicative model, where it reports
exactly its enumeration's single evaluator axiom. The reduction of
Theorem~\ref{thm:meta} is also formalized
...
content to trust. Two further layers are formalized on top. First,
the composed corollary: if $(p+19)/4$ has nine prime factors in
pairwise-distinct unit classes other than $1$ modulo $19$, an
explicit solution of \eqref{eq:es} at $p$ exists ---
Lemma~\ref{lem:S} applied end to end, machine-checked through the
enumeration, both bridges, and the budget-consolidation lemmas.
Second, the $R = 31$ case of Lemma~\ref{lem:S} is verified by
\emph{certified dynamic programming}: the
$\binom{29}{15} = 77{,}558{,}760$ supports are never enumerated ---
a machine-checked soundness induction shows the subset dynamic
program of \texttt{verify\_support\_bound\_dp} covers them all
through $3{,}001$ states, and the only computation trusted to the
evaluator is the final $3{,}001 \times 30$ target check. Every
declaration is audited with \texttt{\#print axioms}. For these enumerations the
sixty-line trust base above is thus replaced by the Lean toolchain
evaluating a formally stated proposition --- and the Python and Lean
implementations, written independently against the same statements,
agree.

\begin{center}
\small
\begin{tabular}{>{\raggedright\arraybackslash}p{2.4cm}|>{\raggedright\arraybackslash}p{5.4cm}|r|>{\raggedright\arraybackslash}p{3.2cm}}
...
\begin{center}
\small
\begin{tabular}{>{\raggedright\arraybackslash}p{3.7cm}|r|>{\raggedright\arraybackslash}p{6.8cm}}
...
computation; consistency with the minimal-$R$ map \\ \hline
...
preprint, arXiv:2509.00128, 2025.
...
\texttt{math.uindy.edu/swett/esc.htm} (accessible via the Internet
Archive).
