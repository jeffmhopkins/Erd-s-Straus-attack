...
The reduction that was prose-only in the finite checks' docstrings
("a budget of twice the cap covers the full cyclic subgroup, so higher
multiplicities are model-equivalent" — the multiplicity cap of the
paper's meta-theorem, Remark 2.6): once an entry's exponent budget
reaches `orderOf x − 1`, its power set is the full cyclic subgroup
`⟨x⟩`, so `reach` is invariant under any further budget increase.
Together with `reach_mono` this makes the capped enumerations lossless,
not just monotone under-approximations. -/

/-- Every unit of `ZMod R` (`R ≠ 0`) has finite order — the classes of
the model are units, so the saturation lemmas below apply to them. -/
lemma isOfFinOrder_of_isUnit [NeZero R] {x : ZMod R} (hx : IsUnit x) :
    IsOfFinOrder x := by
  obtain ⟨u, rfl⟩ := hx
  exact Units.isOfFinOrder_val.mpr (isOfFinOrder_of_finite u)

/-- **Saturation of `powers`**: at any budget `≥ orderOf x − 1` the
power set is the full cyclic subgroup generated by `x`. -/
lemma powers_saturate (x : ZMod R) (hx : IsOfFinOrder x) {b : ℕ}
    (hb : orderOf x - 1 ≤ b) :
    powers x b = (range (orderOf x)).image (x ^ ·) := by
  have hord : 0 < orderOf x := hx.orderOf_pos
  apply Finset.Subset.antisymm
  · intro z hz
    simp only [powers, mem_image, mem_range] at hz ⊢
    obtain ⟨e, _, rfl⟩ := hz
    exact ⟨e % orderOf x, Nat.mod_lt _ hord, pow_mod_orderOf x e⟩
  · intro z hz
    simp only [powers, mem_image, mem_range] at hz ⊢
    obtain ⟨e, he, rfl⟩ := hz
    exact ⟨e, by omega, rfl⟩

/-- Two saturated budgets give the same power set. -/
lemma powers_congr_saturated {x : ZMod R} (hx : IsOfFinOrder x)
    {b b' : ℕ} (hb : orderOf x - 1 ≤ b) (hb' : orderOf x - 1 ≤ b') :
    powers x b = powers x b' := by
  rw [powers_saturate x hx hb, powers_saturate x hx hb']

/-- `step` depends on its entry only through the entry's power set. -/
lemma step_congr (S : Finset (ZMod R)) {xb xb' : ZMod R × ℕ}
    (h : powers xb.1 xb.2 = powers xb'.1 xb'.2) :
    step S xb = step S xb' := by
  simp only [step, h]

/-- `reach` is invariant under entrywise power-set-preserving changes
of the configuration. -/
lemma reach_congr {l l' : List (ZMod R × ℕ)}
    (h : List.Forall₂
      (fun ab ab' => powers ab.1 ab.2 = powers ab'.1 ab'.2) l l') :
    reach l = reach l' := by
  suffices key : ∀ S : Finset (ZMod R), l.foldl step S = l'.foldl step S
    from key {1}
  induction h with
  | nil => intro S; rfl
  | cons hab _ ih =>
...
/-- **Cap saturation for `reach`**: two configurations agreeing
classwise, with all budgets at or above the saturation threshold
`orderOf class − 1`, have the same reach. -/
theorem reach_cap_saturate {l l' : List (ZMod R × ℕ)}
    (h : List.Forall₂ (fun ab ab' => ab.1 = ab'.1 ∧ IsOfFinOrder ab.1 ∧
      orderOf ab.1 - 1 ≤ ab.2 ∧ orderOf ab.1 - 1 ≤ ab'.2) l l') :
    reach l = reach l' := by
  refine reach_congr (h.imp ?_)
  rintro ⟨x, b⟩ ⟨x', b'⟩ ⟨heq, hfin, hb, hb'⟩
  cases heq
  exact powers_congr_saturated hfin hb hb'

/-- Single-entry form: overshooting one budget past saturation never
changes `reach`. -/
theorem reach_cap_entry (l₁ l₂ : List (ZMod R × ℕ)) {x : ZMod R}
    (hx : IsOfFinOrder x) {b b' : ℕ}
    (hb : orderOf x - 1 ≤ b) (hb' : orderOf x - 1 ≤ b') :
    reach (l₁ ++ (x, b) :: l₂) = reach (l₁ ++ (x, b') :: l₂) := by
  refine reach_congr (List.rel_append ?_ (List.Forall₂.cons ?_ ?_))
  · exact List.forall₂_same.mpr fun _ _ => rfl
  · exact powers_congr_saturated hx hb hb'
  · exact List.forall₂_same.mpr fun _ _ => rfl

/-- **The multiplicity cap is lossless** (the meta-theorem's cap,
formally): if the cap `c` satisfies `orderOf x ≤ 2c + 1` (true for
`c = ⌈(R−1)/2⌉` and any unit class, whose order divides `R − 1`),
then a multiplicity `μ ≥ c` — budget `2μ` — reaches exactly what the
capped multiplicity `c` — budget `2c` — reaches. -/
theorem reach_mult_cap (l₁ l₂ : List (ZMod R × ℕ)) {x : ZMod R}
    (hx : IsOfFinOrder x) {c μ : ℕ}
    (hcap : orderOf x ≤ 2 * c + 1) (hμ : c ≤ μ) :
    reach (l₁ ++ (x, 2 * μ) :: l₂) = reach (l₁ ++ (x, 2 * c) :: l₂) :=
  reach_cap_entry l₁ l₂ hx (by omega) (by omega)
...
#print axioms ErdosStraus.powers_saturate
#print axioms ErdosStraus.reach_cap_saturate
#print axioms ErdosStraus.reach_mult_cap
