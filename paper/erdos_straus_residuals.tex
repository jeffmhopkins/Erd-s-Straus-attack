...
\textup{(A1)} $\legendre{q}{r_1} = +1$;
\textup{(A2)} $\legendre{\ell}{r_1} = +1$ for \emph{every} prime
$\ell \le q$ that the class forces to divide $a$;
\textup{(A3)} either $r_1 \equiv 3 \pmod 8$ with $a$ odd, or
...
$-r_1$; condition~\textup{(A3)} is what makes the $2$-adic
constraint solvable, and~\textup{(A2)} is what makes the family
nonempty. One point is not a routine citation: the sieve delivers
...
Condition \textup{(A2)} is restrictive in a way that is easy to miss:
the class fixes $a$ modulo every prime $\le q$, so a single forced
factor $\ell$ with $\legendre{\ell}{r_1} = -1$ empties the family.
For instance $q = 23$, $R_0 = 35$, $r_1 = 7$ satisfies
\textup{(A1)}, but $3 \mid a$ for every hard prime (as
$p \equiv 1 \pmod 3$ and $35 \equiv 2 \pmod 3$) while
$\legendre{3}{7} = -1$: the set is empty. Counting properly, among
hard primes below $2 \times 10^7$ the classes passing
\textup{(A1)} account for $6.6\%$ of first rungs, but those passing
\textup{(A1)}--\textup{(A3)} with a nonempty family account for only
$0.023\%$. Theorem~\ref{thm:P1} is therefore an existence statement
about a sparse family of ladders, not a bound covering a positive
proportion of them; and whenever $R_0$ is prime --- which is the
case for the large majority of hard primes --- no $r_1$ of this kind
exists at all. Consequently the bracket on Hypothesis~\ref{hyp:P}
