...
\item A machine-checked (Lean~4/mathlib) verification of the elementary
  ...
  Theorems~\ref{thm:Ap} and~\ref{thm:App}, and the support-bound
  checks of Lemma~\ref{lem:S} at $R = 19$ and $23$ --- in the
...
Theorem~\ref{thm:Ap} (re-stated multiplicatively over
$(\mathbb{Z}/7)^\times$), the $R = 11$ criterion of
Theorem~\ref{thm:App}, and the $R = 19, 23$ cases of
Lemma~\ref{lem:S} (the latter three in the discrete-log coordinates
of their proofs) are discharged by Lean's compiled evaluator, with
the monotonicity reductions (Lemma~\ref{lem:mono}) proved as theorems
...
Theorem~\ref{thm:Ap} ($1{,}536$ configurations, stated in the
multiplicative model itself), Theorem~\ref{thm:App} ($497{,}664$
capped configurations, including the exact two-case failure
classification), and the $R = 19, 23$ cases of Lemma~\ref{lem:S}
($437{,}580$ resp.\ $7{,}759{,}752$ checks) --- the latter three
stated in the discrete-log coordinates in which the proofs and
\texttt{verify\_support\_bound} work --- are discharged by Lean's
compiled evaluator
(\texttt{native\_decide}, which trusts the Lean compiler in addition
to the kernel); every declaration is audited with
\texttt{\#print axioms}. For these two enumerations the sixty-line
trust base above is thus replaced by the Lean toolchain evaluating a
formally stated proposition --- and the Python and Lean
