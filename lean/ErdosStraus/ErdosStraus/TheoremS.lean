...
theorem theoremS_support_bound {d : ℕ} [NeZero d] (hd : Even d)
  ...
  · -- trivial stabilizer: `#(A v) ≥ 3` except at most one 2-torsion class
    ...
    have hkn : ∑ v ∈ S, #(A v) + 1 ≤ #M + #S := by
      ...
      exact h
    -- nonzero 2-torsion is unique (`v = d/2`)
    have htors : ∀ v ∈ S, 2 * v = 0 → 2 * v.val = d := by
      intro v hv h2
      have hneg : -v = v := by
      ...
      · exact hval
    have hS0le : #(S.filter fun v => 2 * v = 0) ≤ 1 := by
    ...
    have hA2 : ∀ v ∈ S, 2 ≤ #(A v) := by
      ...
      have hsub : ({0, v} : Finset (ZMod d)) ⊆ A v := by
        ...
        tauto
      have h2 : #({0, v} : Finset (ZMod d)) = 2 := by
        rw [Finset.card_insert_of_notMem (by
          simp only [Finset.mem_singleton]
          exact fun h => hv0 h.symm), Finset.card_singleton]
      calc 2 = #({0, v} : Finset (ZMod d)) := h2.symm
        _ ≤ #(A v) := Finset.card_le_card hsub
    have hA3 : ∀ v ∈ S, ¬ 2 * v = 0 → 3 ≤ #(A v) := by
      intro v hv h2v
      have hv0 : v ≠ 0 := by rintro rfl; exact hS0 hv
    ...
    have hsum_split := Finset.sum_filter_add_sum_filter_not S
      (fun v => 2 * v = 0) (fun v => #(A v))
    have hcard_split : #(S.filter fun v => 2 * v = 0)
        + #(S.filter fun v => ¬ 2 * v = 0) = #S :=
      Finset.card_filter_add_card_filter_not (fun v => 2 * v = 0)
    have hbb1 : 2 * #(S.filter fun v => 2 * v = 0)
        ≤ ∑ v ∈ S.filter (fun v => 2 * v = 0), #(A v) := by
      have h := Finset.card_nsmul_le_sum (S.filter fun v => 2 * v = 0)
        (fun v => #(A v)) 2 (fun v hv => hA2 v (Finset.mem_of_mem_filter v hv))
      rwa [smul_eq_mul, mul_comm] at h
    have hbb2 : 3 * #(S.filter fun v => ¬ 2 * v = 0)
        ≤ ∑ v ∈ S.filter (fun v => ¬ 2 * v = 0), #(A v) := by
      have h := Finset.card_nsmul_le_sum (S.filter fun v => ¬ 2 * v = 0)
      ...
      rwa [smul_eq_mul, mul_comm] at h
    omega
  · -- nontrivial stabilizer of order `h`: `2 ≤ h`, `h ∣ d`
    ...
    -- the quadratic endgame: `k ≤ h + m − 3 ≤ d/2 − 1`
    obtain ⟨h2, hh2⟩ : ∃ h2, #M.addStab = h2 + 2 :=
      ⟨#M.addStab - 2, by omega⟩
    obtain ⟨m2, hm2'⟩ : ∃ m2, m = m2 + 2 := ⟨m - 2, by omega⟩
    obtain ⟨q, hq⟩ : ∃ q, h2 * m2 = q := ⟨_, rfl⟩
    have hd_expand : d = q + 2 * h2 + 2 * m2 + 4 := by
