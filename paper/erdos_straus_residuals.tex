...
solution arises this way. Solvability at any fixed residual --- prime
or composite --- is a computable function of finite data, yielding
exact criteria at $R = 3, 7, 11, 15$, and a reciprocity identity
$\smash{\legendre{q}{R}} = \smash{\legendre{p}{q}}$ governs how
failures correlate across residuals. Failure at $R$ forbids at least
half the unit classes mod $R$ on the shifted form $(p+R)/4$ --- proved
for every residual at once via Kneser's addition theorem --- giving
exceptional sets
$O_{\mathcal{P}}(x/(\log x)^{1+|\mathcal{P}|/2})$ for every finite
admissible residual list $\mathcal{P}$ drawn from the primes and the
composites up to $107$ (marginally weaker exponents beyond): weaker in
density than
Vaughan's classical $x\exp(-c(\log x)^{2/3})$, but explicit and
certificate-producing, with the elementary layer and the core finite
enumerations of the proofs machine-checked in Lean~4/\mbox{mathlib}.
Letting the residual list grow with $x$ yields an unconditional
almost-all theorem: all but $O(x\exp(-c(\log\log x)^2))$ hard primes
$p \le x$ satisfy $R_{\min}(p) \le \varepsilon \log\log x$; a
branch-classification conjecture --- kernel case proved, aperiodic
case verified exhaustively at small moduli and at nine larger ones
--- would raise this to $R_{\min}(p) \le (\log x)^{c_0}$.
A reciprocity-selected \emph{ladder} of $p$-adapted residuals then
localizes the conjecture for hard primes into a single measured
counting inequality, with the first rung proved and a matching
half-dimensional lower bound for true failures given in outline. Computationally we exhibit
certificates for all $128{,}671{,}219$
...
Thm.~\ref{thm:Upay} & Conjecture~\ref{conj:A} + the audit of Thm.~\ref{thm:U} & \S\ref{sec:sieve} & no (conditional) \\ \addlinespace
...
\cite[Ch.~2]{HalberstamRichert} at $s \asymp \kappa$: since the
...
(Theorem~\ref{thm:Upay}), with the intrinsic family ceiling at
exponent $\tfrac12$. Vaughan's bound
...
concentration: maximal failing supports exist at every size from
...
complete partitions, and each has reach exactly
...
for odd $\bar d$ no summand is degenerate and Kneser's bound gives
$|T| \le (\bar d - 3)/2$.)
\end{theorem}
...
family $\{C(K, W) : |C(K, W)| \le \lfloor d/2 \rfloor - 1\}$ has at
most $2d^2$ members.
...
\emph{all} targets ($46{,}000$-plus maximal supports; the bound
...
Every failing support for $(d, t)$ is contained in one of at most
$2d^2$ explicit window containers of size
$\le \lfloor d/2 \rfloor - 1$; hence failure of residual $R$
confines the factor classes of $(p+R)/4$ to one of $O(R^2)$
branches, each forbidding at least half the nonzero classes ---
sieve dimension $\tfrac12$ per residual, with the branch count
reduced from $2^{\varphi(R)}$ to $O(R^2)$.
\end{theorem}
...
$e^{\Theta(B^2)}$ to $\prod_{R \le B} O(R^2) =
e^{(2+o(1)) \cdot \frac{B}{4}\log B}$; the class conditioning
...
$(2 + c_M)\theta L$ ($L = \log\log x$, $\delta = \tfrac12$),
admissible for every
$\theta < \delta/(2 + c_M + \delta)$ --- with the constants proved
here, any $\theta$ below roughly $1/7$. The family ceiling is
$c_0 < \tfrac12$: the container-count exponent cannot reach $0$. A
