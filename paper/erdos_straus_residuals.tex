...
admissible residual list $\mathcal{P}$ (entries prime or composite
$\le 107$; marginally weaker exponents beyond): weaker in
density than
...
no other part and $T'$ is recoverable from the support), and each
has reach exactly
...
$(3 + c_M)\theta L$ ($L = \log\log x$, $\delta = \tfrac12$),
admissible for every
$\theta < \delta/(3 + c_M + \delta)$ --- with the constants proved
here, any $\theta$ below roughly $1/9$. The family ceiling is
