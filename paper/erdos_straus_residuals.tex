...
admissible residual list $\mathcal{P}$: weaker in density than
Vaughan's classical $x\exp(-c(\log x)^{2/3})$, but explicit and
certificate-producing, with the elementary and finite-enumeration
layers of the proofs machine-checked in Lean~4/\mbox{mathlib}.
...
counting inequality, with the first rung proved and failures bounded
below by the half-dimensional sieve. Computationally we exhibit
...
\item The \emph{Burgess--reciprocity ladder}
  ...
  two-sided (Theorem~\ref{thm:B2}), true failures bounded below via
  the half-dimensional sieve (Theorem~\ref{thm:P1}), and an
...
Theorem~\ref{thm:D}. Section~\ref{sec:ladder} develops the
Burgess--reciprocity ladder on top of Theorem~\ref{thm:S} and the
census of Section~\ref{sec:comp}.
...
Thm.~\ref{thm:Ap} ($R{=}7$) & Thm.~\ref{thm:meta} + enumeration ($1{,}536$) & \S\ref{sec:exact} & Lean (\texttt{native\_decide}) \\ \addlinespace
...
Thm.~\ref{thm:S} & Kneser's theorem & \S\ref{sec:sieve} & no \\ \addlinespace
...
Thm.~\ref{thm:L1} & Hypothesis~\ref{hyp:B} (measured) & \S\ref{sec:ladder} & no (conditional) \\
\bottomrule
\end{tabular}
\caption{Audit map: what each main result depends on, where it is
proved, and its formal-verification status.}
\label{tab:deps}
\end{table}
...
Theorem~\ref{thm:S} closed the fixed-list axis: every residual
contributes its maximal sieve input, and no fixed finite list can
reach emptiness (Remark~\ref{rem:emptiness}). This section develops
the program that opens in its place: a \emph{$p$-adapted} family of
...
Burgess bounds $q \ll p^{1/(4\sqrt e)+\varepsilon}$ unconditionally
and GRH gives $q \ll (\log p)^2$; the observed maximum below
...
rung and along the $q$-ladder. The per-rung failure rate is flat in
$p$ across two decades, including on the adversarial deep-tail
population.}
\label{tab:ladder}
\end{table}
...
$O_{q,J}\!\left(x/(\log x)^{1+J'/2}\right)$, where $J'$ is the number
of rungs with at most two prime factors.
\end{theorem}
...
condition of dimension $\ge \tfrac12$, exactly as in the proof of
...
$R_0$ is $\ll_q (\log x)^{-1/\varphi(4q)} \to 0$.
\end{theorem}

\begin{proof}
The condition $q(p) = q$ is a union of residue classes modulo
$M = 840\prod_{11 \le \ell \le q}\ell$; fix one, which fixes $R_0$.
By Proposition~\ref{prop:succ}, any prime factor of $(p+R_0)/4$ in
the universal class $-4^{-1} \bmod R_0$ certifies success via
...
against class size $\asymp x/\log x$ (Siegel--Walfisz). The bound is
uniform for $q \ll \log\log x$.
\end{proof}

\begin{theorem}[proxy ladder, two-sided]\label{thm:B2}
Fix $q$, a class \textup(fixing $R_0, R_1, \dots$\textup), and $J$
with $\kappa = \sum_{j<J} 1/\varphi(R_j)$ below an absolute
constant. Let $\tilde E_J(x)$ be the set of hard primes in the class
whose shifted values $(p+R_j)/4$, $j < J$, all avoid prime factors
in their universal classes. Then
\[
...
There is $c_P > 0$ with $\#E_j(x) \ge c_P\, \#\tilde E_j(x)$ for
$j \le J$ and $x$ large, where $E_j$ is the set of true ladder
failures through rung $j$: failures are a positive proportion of
their necessary-condition set.
\end{hypothesis}

Measured: $c_P \approx 0.10$--$0.12$ at rung $0$ and $0.13$--$0.16$
...
\]
so the ladder failure count decays at \emph{every} fixed rung --- the
per-rung decay Hypothesis~\ref{hyp:B} below posits, proved from
Hypothesis~\ref{hyp:P} alone.

\begin{theorem}[half-dimensional failure lower bound]\label{thm:P1}
Call a class admissible if $R_0$ is composite with a prime factor
$r_1 \equiv 3 \pmod 4$ such that $\legendre{q}{r_1} = +1$ \textup(and
$\legendre{2}{r_1} = +1$ when $2 \mid a$\textup). On every admissible
class,
\[
  \#E_0(x) \;\ge\; \#\{p \equiv c \le x : \text{every prime factor
  of } (p+R_0)/4 \text{ is a QR mod } r_1\} \;\gg\;
  \frac{x}{(\log x)^{3/2}}.
\]
\end{theorem}
...
There are $\delta > 0$ and $x_0$ such that for $x \ge x_0$, all
...
from Hypothesis~\ref{hyp:P}; the measured survival factors are
$0.86$--$0.95$ per rung with no drift in $p$.
...
$\ll x\exp(-\delta(\log\log x)^2)$, beyond every fixed power of
$\log x$ and hence beyond Theorems~\ref{thm:FG} and~\ref{thm:D};
with $J(x) = (\log x)^{\theta}$, $\theta > 2/3$, it would pass
...
localize the conjecture for hard primes into one measured, twice-
bracketed counting inequality, with every sub-statement current
tools can decide now decided.
\end{remark}
...
\texttt{meta.json} (prefixes \texttt{53cf17c4\ldots},
