...
$1 + \tfrac12 + \tfrac12 + \tfrac35 = \tfrac{13}{5}$, and the Fundamental
Lemma gives $O(x/(\log x)^{5/2})$ plus a lower-order term.
...
combined branches, each a five-form problem of dimension $\ge 3$; the
...
\begin{proof}[Proof of Theorem \ref{thm:D} (sketch)]
By Proposition~\ref{prop:succ}, failure at $R$ implies $(p+R)/4$ has no
prime factor in at least one explicit class mod $R$, a sifting condition
of dimension $\ge 1/\varphi(R)$ on the form $(p+R)/4$; distinct $R$ give
distinct shifts. Since $\sum_{R \equiv 3 (4)} 1/\varphi(R)$ diverges, a
finite set $S(A)$ achieves total dimension $> A$, and the
multidimensional Selberg sieve yields the claim.
\end{proof}
...
\item No prime has minimal $R \in \{87, 91, 95, 99, 103\}$. This gap
  ...
  model the expected counts there are $0.12, 0.03, 0.05, 0.01, 0.01$.
