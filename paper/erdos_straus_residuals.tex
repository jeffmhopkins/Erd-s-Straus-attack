...
residual $R$ --- prime or composite --- is a computable function of the factor classes of $(p+R)/4$
and of $p \bmod R$ (a finite-state meta-theorem), yielding closed-form
criteria at $R = 3, 7, 11$ and at the first composite residual $R = 15$
(a clean Jacobi-character dichotomy), and a reciprocity identity
...
shifted form $(p+R)/4$ --- is proved \emph{unconditionally for every
residual at once} via Kneser's addition theorem. Consequences include an
unconditional chain of sieve bounds --- exceptional sets
$O_{\mathcal{P}}(x/(\log x)^{1+|\mathcal{P}|/2})$ for \emph{every}
finite admissible residual list $\mathcal{P}$, reaching exponent $31/2$
on the $27$ residuals up to $107$ --- each cut out by
...
together with the forced powers of $2$, always reach the target. The
criterion agrees with ground-truth divisor search for $158{,}759$ of
$158{,}759$ hard primes tested below $10^9$.
\end{proof}
...
A$''$ $=$ Theorem~\ref{thm:App}, A$'''$ $=$ Theorem~\ref{thm:R15},
Theorem~J $=$ Theorem~\ref{thm:J},
Theorem~I $=$ Proposition~\ref{prop:agg}, Lemma~S $=$
Lemma~\ref{lem:S}, Theorem~S $=$ Theorem~\ref{thm:S}, and the
monotonicity reduction $=$ Lemma~\ref{lem:mono}.
